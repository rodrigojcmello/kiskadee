...
The Showcase keeps one effective icon family and variant in `IconFamilyProvider`. Its existing
selector changes both free family-resolved examples and the Essential Icons used by component
affordances; there is no second family state.

Menu example data may store `IconName`, but React adapters receive an explicit `renderIcon` that
composes `FamilyResolvedIcon`. Direct Consumer-provided Icons can instead render arbitrary nodes
and do not require the family provider. See the cross-package
[`icon-consumption`](../../docs/definitions/icon-consumption.md) definition.
...
- Showcase-owned interface glyphs resolve through a nested public `IconFamilyProvider` fixed to
  `carbon.regular`. The lateral icon-family selector projects every available family and local
  variant combination from `@kiskadee/icons`' public lazy catalog and changes only component
