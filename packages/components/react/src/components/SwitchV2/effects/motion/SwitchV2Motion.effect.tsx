import './SwitchV2Motion.structural.css';
import type { SwitchActivationMotion } from '@kiskadee/core';
import { animate, motion, useMotionValue } from 'motion/react';
import { type ReactNode, type RefObject, useCallback, useEffect, useRef, useState } from 'react';
import { join } from '../../../Switch/Switch.class-names.ts';
import type { SwitchClassNames } from '../../../Switch/Switch.types.ts';
...
const SWITCH_V2_MOTION_EXTREMITY_EPSILON = 0.5;
const SWITCH_V2_MOTION_VELOCITY_PROJECTION = 0.18;
const SWITCH_V2_MOTION_THUMB_TRANSITIONS = {
  standard: {
    type: 'spring',
    stiffness: 520,
    damping: 38,
...
function resolveControlStateAtExtremity(
  ...
  if (normalizedOffset <= SWITCH_V2_MOTION_EXTREMITY_EPSILON) return false;
  if (normalizedOffset >= translation - SWITCH_V2_MOTION_EXTREMITY_EPSILON) return true;
  return null;
}

export function resolveSwitchV2MotionEffect(
...
export function SwitchV2MotionThumb({
  activationMotion,
  children,
  controlState,
  disabled,
  ...
  const thumbTransition = SWITCH_V2_MOTION_THUMB_TRANSITIONS[activationMotion];
    ...
    <motion.span
      ...
      onDragEnd={(_, info) => {
        ...
        const projectedOffset =
          currentOffset + info.velocity.x * inlineDirection * SWITCH_V2_MOTION_VELOCITY_PROJECTION;
        const nextControlState = projectedOffset >= thumbTranslation / 2;

        isDraggingRef.current = false;
        if (hasDraggedRef.current || Math.abs(info.offset.x) > SWITCH_V2_MOTION_DRAG_THRESHOLD) {
          requestSuppressNextClick();
        }
