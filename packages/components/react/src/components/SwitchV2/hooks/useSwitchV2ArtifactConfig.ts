...
import type { SwitchVariantClassesMap } from '../../Switch/Switch.types.ts';
import {
  type SwitchV2ThumbSizeEffectModule,
  useSwitchV2ThumbSizeEffect
} from '../effects/thumb-size/index.ts';

export type SwitchV2ArtifactConfig = {
  ...
  effects: {
    thumbSizeEffect: SwitchV2ThumbSizeEffectModule | null;
  };
  globalEffects: {
...
export function useSwitchV2ArtifactConfig(thumbSize?: false): SwitchV2ArtifactConfig {
  ...
  const thumbSizeEffect = useSwitchV2ThumbSizeEffect(shouldLoadThumbSizeEffect);
