 ...
 *    rewrite TS extensions in emitted `.js`).
 *  - TypeScript's `rewriteRelativeImportExtensions` does not rewrite specifiers
 *    inside emitted `.d.ts` (microsoft/TypeScript#61037).
 *
 * Scope: only relative specifiers (`./` or `../`). Bare specifiers and aliases
 * are left untouched.
 */
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  rewriteDistExtensions().catch((error) => {
    console.error('[react-components] Failed to rewrite dist extensions:', error);
    process.exitCode = 1;
  });
}
