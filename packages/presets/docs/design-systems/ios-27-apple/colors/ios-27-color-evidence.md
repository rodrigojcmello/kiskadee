...
is byte-exact. This candidate does not replace the approved
[`figma-to-kiskadee.json`](figma-to-kiskadee.json) mapping or the promoted preset assets.
...
evaluated locally with `@kiskadee/tonal-scale@0.7.0`. It moves Apple's tinted Gray source from
...
assets listed above remain the approved `0.4.1` preset inputs. The current viewer retains only the
two harmony-generated companions that have no Apple Accent identity: Lime and Magenta.
...
Generator `0.7.0` recognizes Apple Brown as `yr.brown.v1`. This changes the current candidate and
its viewer recipe, but does not rewrite the previously approved `0.4.1` files or their provenance.
...
The current multifamily contract accepts one seed per family. Consequently, Apple Light Accents
remain exact source anchors while the distinct Apple Dark Accent values map to the nearest tone on
the adaptive Dark track. The candidate mapping makes those differences explicit; Yellow and Purple
currently have the largest Dark Delta E and require visual review before any V5 asset promotion.
