...
export function generateColorScaleMaterialLike(hexColor: string): EmphasisLevel {
  const palette = CorePalette.of(argbFromHex(hexColor)).a1;

  const subtle: ColorScaleLight = {};
  const vivid: ColorScaleDark = {};

  const resolveHslaAtTone = (tone: number): HSLA => {
    const materialTone = canonicalLightnessAtTone(tone);
...
export function generateColorScaleMaterialLikeArtifact(hexColor: string): EmphasisLevel {
  const tracks = generateColorScaleMaterialLike(hexColor);
  ...
  const fallbackDarkness = 100 - inputLightness;
  const fallbackTone = snapSupportedTone(fallbackDarkness) as number;
  ...
  const header = `// Generated by generateColorScaleMaterialLikeArtifact('${hexColor}')\n`;
