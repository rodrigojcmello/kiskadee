...
export function generateColorScale(
  ...
  // NOTE: The subtle track (0–30) must always keep a fixed, inverted mapping:
  //   tone + lightness = 100
  // This keeps early tones stable and predictable, and prevents the anchor color
  // (tone 50) from affecting tones 20–30.
  for (let tone = 0; tone <= 30; tone += 1) {
    const lightness = 100 - tone;
    scale[tone as keyof ColorScale] = [hue, saturation, lightness, alpha];
  }

  // From 35 onwards we allow the anchor (tone 50) to influence the curve.
  // We generate base stops that the vivid track will later interpolate from.
  //
  // Stops:
  // - tone 30: L=70 (fixed)
  // - tone 50: L=anchorLightness (original input, unless re-centered)
  // - tone 100: L=0
  const stop30Lightness = 70;

  const lightnessAtTone = (tone: number): number => {
    if (tone <= 30) return 100 - tone;
    if (tone < 50) {
      // Interpolate from 30→50
      const ratio = (tone - 30) / (50 - 30);
      return Math.round(stop30Lightness + (anchorLightness - stop30Lightness) * ratio);
    }
    // Interpolate from 50→100
    const ratio = (tone - 50) / (100 - 50);
    return Math.round(anchorLightness + (0 - anchorLightness) * ratio);
  };

  const baseStopsForScale = [30, 40, 50, 60, 70, 80, 90, 100] as const;
  for (const tone of baseStopsForScale) {
    scale[tone as keyof ColorScale] = [hue, saturation, lightnessAtTone(tone), alpha];
  }
  ...
  // Base stops must exist in `scale` for vivid interpolation.
  // They are generated above using the anchor-aware curve.
 ...
 *
 * @param hexColor - Hexadecimal color string (e.g., "#6750A4")
 * @param forceMidToneLightness50 - When true, ignore the original lightness and center emphasis 50 at 50.
 ...
 * @param invertScale
 * @example
 * generateColorScaleArtifact("#6750A4", true);
 * // Logs the scale structure and returns the ColorScale object
 */
export function generateColorScaleArtifact(
  hexColor: string,
  forceMidToneLightness50 = false,
  ...
  const rawOverrides = rawArgs[2] as unknown;
  const rawShaping = rawArgs[3] as unknown;
  const rawInvert = rawArgs[4] as unknown;

  // Backwards-compatible overload:
  // - generateColorScaleArtifact(hex, force, overrides, invertScale)
  // - generateColorScaleArtifact(hex, force, overrides, shaping, invertScale)
  //
  // The header emitted in the generated file documents the effective call
  // shape so it can be copy-pasted back into this script.
  if (typeof shaping === 'boolean') {
    invertScale = shaping;
    shaping = undefined;
  }

  const baseTracks = generateColorScale(hexColor, forceMidToneLightness50, invertScale);
  ...
  const isPrioritizingScale = forceMidToneLightness50;

  // Recompute original lightness to derive an approximate darkness label.
  // For forceMidToneLightness50=false this is used only to document the
  // anchor color (emphasis 50). For forceMidToneLightness50=true we also use it
  ...
  for (const tone of softKeysForLog) {
    ...
    const comment = (() => {
      // Subtle track comments mirror the vivid track behaviour for overrides: any
      ...
      // Non-overridden subtle tones keep the existing simple darkness label.
      if (t === 0) {
        const zeroLabel = invertScale
          ? '100% darkness (black/darkest)'
          : '0% darkness (white/lightest)';
  ...
  const rawArgsLength = rawArgs.length;
  const overridesWasPassed = rawArgsLength >= 3;
  const shapingWasPassed = rawArgsLength >= 4;
  const invertWasPassed = rawArgsLength >= 5;

  const shapingIsBooleanOverload = shapingWasPassed && typeof rawShaping === 'boolean';
  const effectiveInvertScale: boolean = invertWasPassed
    ? Boolean(rawInvert)
    : shapingIsBooleanOverload
      ? Boolean(rawShaping)
      : invertScale;

  const overridesIsObject =
    rawOverrides !== null && typeof rawOverrides === 'object' && !Array.isArray(rawOverrides);

  const overridesEntries = overridesIsObject
    ? Object.entries(rawOverrides as Record<string, unknown>).sort(
        ([a], [b]) => Number(a) - Number(b)
      )
    : [];
  const hasOverrides = overridesIsObject && overridesEntries.length > 0;

  // Header must reflect the *actual* call arguments, including explicit `undefined`.
  // This makes the file self-reproducible even when the caller intentionally uses
  // `undefined` placeholders to skip optional parameters.
  if (!shaping || shapingIsBooleanOverload) {
    // No shaping object in effect.
    // Prefer the historical 4-arg documented form when the caller did not pass a
    // shaping placeholder.

    if (!overridesWasPassed) {
      headerLines.push(
        `// Generated by generateColorScaleArtifact('${hexColor}', ${forceMidToneLightness50}, {}, ${effectiveInvertScale})`
      );
    } else if (rawOverrides === undefined) {
      if (shapingWasPassed && rawShaping === undefined) {
        // Caller explicitly passed `undefined` placeholders.
        headerLines.push(
          `// Generated by generateColorScaleArtifact('${hexColor}', ${forceMidToneLightness50}, undefined, undefined, ${effectiveInvertScale})`
        );
      } else {
        // Caller passed undefined overrides but no shaping placeholder.
        headerLines.push(
          `// Generated by generateColorScaleArtifact('${hexColor}', ${forceMidToneLightness50}, undefined, ${effectiveInvertScale})`
        );
      }
      ...
      headerLines.push(
        `// Generated by generateColorScaleArtifact('${hexColor}', ${forceMidToneLightness50}, {`
      );
      for (const [tone, hex] of overridesEntries) {
        headerLines.push(`//   ${tone}: '${String(hex)}',`);
      }
      headerLines.push(`// }, ${effectiveInvertScale})`);
    }
  } else {
    // 5-argument form with shaping:
    // generateColorScaleArtifact(hex, forceMidToneLightness50, overrides, shaping, invertScale)
    if (!overridesWasPassed || rawOverrides === undefined || !hasOverrides) {
      const overridesExpr = overridesWasPassed && rawOverrides === undefined ? 'undefined' : '{}';
      headerLines.push(
        `// Generated by generateColorScaleArtifact('${hexColor}', ${forceMidToneLightness50}, ${overridesExpr}, {`
      );
    } else {
      headerLines.push(
        `// Generated by generateColorScaleArtifact('${hexColor}', ${forceMidToneLightness50}, {`
    ...
    headerLines.push(`//   anchorTone: ${(rawShaping as DarkToneShaping).anchorTone},`);

    const shapingObj = rawShaping as DarkToneShaping;
    if (typeof shapingObj.hueShiftTotal === 'number') {
      headerLines.push(`//   hueShiftTotal: ${shapingObj.hueShiftTotal},`);
    }
    if (typeof shapingObj.saturationDropTotal === 'number') {
      headerLines.push(`//   saturationDropTotal: ${shapingObj.saturationDropTotal},`);
    }
    if (typeof shapingObj.lightnessStep === 'number') {
      headerLines.push(`//   lightnessStep: ${shapingObj.lightnessStep},`);
    }
  ...
  console.log(`[generateColorScaleArtifact] Wrote TS to: ${outFilePath}`);
...
// generateColorScaleArtifact('#0F6CBD', true, {
//   70: '#115EA3',
//   80: '#0F548C',
//   90: '#0C3B5E'
// });
...
// Fluent Primary - Dark
generateColorScaleArtifact('#8283F6', false, undefined, undefined, false);
