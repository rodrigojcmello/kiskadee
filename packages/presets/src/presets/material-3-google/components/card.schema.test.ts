...
const { card, container } = createMaterial3GoogleCardSchema({
  c,
  ...
  for (const segment of ['default', 'dynamic', 'purple'] as const)
    for (const theme of ['light', 'dark'] as const)
      for (const surface of ['onSubtle', 'onVivid'] as const) {
        const entries = container.options?.canonicalSurfaces?.[segment]?.[theme];
        expect(entries).toHaveLength(10);
        for (const entry of entries!) {
          const rest =
            container.elements.e1?.palettes?.[segment]?.[theme]?.[surface]?.boxColor?.[
              entry.intent
            ]?.[entry.emphasis]?.rest;
          const states =
          ...
          expect(rest).toMatch(/^#/);
          expect(states?.rest).toBeUndefined();
          expect(states?.hover).not.toEqual(rest);
          expect(
            container.contentSurfaceContext?.[segment]?.[theme]?.[surface]?.[entry.intent]?.[
              entry.emphasis
            ]?.rest
          ).toBe(entry.contentSurfaceContext);
          expect(
...
it('resolves each medium surface from its own family and keeps border optional', () => {
  const palette = container.elements.e1?.palettes?.default?.light?.onSubtle;
  ...
  for (const segment of ['default', 'dynamic', 'purple'] as const)
    for (const surface of ['onSubtle', 'onVivid'] as const)
      for (const intent of ['neutral', 'primary', 'support'] as const) {
        const light =
          container.elements.e1?.palettes?.[segment]?.light?.[surface]?.boxColor?.[intent];
        const dark =
          container.elements.e1?.palettes?.[segment]?.dark?.[surface]?.boxColor?.[intent];
        const lightStates =
          card.elements.e1?.palettes?.[segment]?.light?.[surface]?.boxColor?.[intent];
        expect(light?.lowest?.rest).toBe('#ffffff');
        expect(lightStates?.lowest?.hover).toBe(c(segment, 'l', 'primitive.black.v1', 1));
  ...
  for (const segment of ['default', 'dynamic', 'purple'] as const)
    for (const theme of ['light', 'dark'] as const) {
      ...
      for (const surface of ['onSubtle', 'onVivid'] as const) {
        ...
        expect(
          container.elements.e1?.palettes?.[segment]?.[theme]?.[surface]?.boxColor?.primary?.highest
            ?.rest
        ).toBe(c.ref(segment, 'l', 'card.primary', 'vivid'));
        ...
        expect(
          container.contentSurfaceContext?.[segment]?.[theme]?.[surface]?.primary?.highest?.rest
        ).toBe('onVivid');
      }
    }
});
...
it('separates optional support from achromatic neutral in every context', () => {
  for (const segment of ['default', 'purple', 'dynamic'] as const)
    for (const theme of ['light', 'dark'] as const)
      for (const surface of ['onSubtle', 'onVivid'] as const) {
        const palettes = container.elements.e1?.palettes?.[segment]?.[theme]?.[surface];
    ...
    for (const theme of ['light', 'dark'] as const)
      for (const surface of ['onSubtle', 'onVivid'] as const)
        for (const intent of ['primary', 'support'] as const) {
          const states =
            container.elements.e1?.palettes?.[segment]?.[theme]?.[surface]?.boxColor?.[intent];
          const interactions =
