...
- Brand and monochrome presentations of the same export must use the exact same `viewBox`. A
  presentation switch changes color treatment, never the public icon size.
...
| `ClaudeIcon` | Claude | [Anthropic press kit](https://www.anthropic.com/press-kit) (`ClaudeIcon-Rounded.svg`) |
...
- Reddit uses the current two-color conversation-bubble icon and the official one-color Snoo
  silhouette from Reddit's brand system. The two presentations share the same `256 × 256`
  coordinate box while keeping their distinct official constructions.
- Google and Microsoft retain their required multicolor identity in `brand`. Claude retains its
  official Clay presentation using the rounded app icon distributed in Anthropic's press kit.
  Gemini retains its official aurora color direction in a compact vector treatment. Their
...
- Reddit's compact export preserves the official Snoo geometry and OrangeRed identity. Its
  monochrome presentation renders the same geometry as one color.
