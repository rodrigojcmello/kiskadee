...
export type MessengerIconConstruction = 'mark';
export type MessengerIconPresentation = 'brand' | 'monochrome';

export type MessengerIconProps = IconProps & (
  | {
      construction?: 'mark';
...
const DEFAULT_PRESENTATIONS: Record<MessengerIconConstruction, MessengerIconPresentation> = {"mark":"brand"};
...
export function MessengerIcon({
  ...
  const resolvedConstruction = construction as MessengerIconConstruction;
  ...
  throw new Error(
    `Unsupported MessengerIcon construction/presentation: ${resolvedConstruction}.${String(resolvedPresentation)}`
  );
}
