...
const showcaseRegistryDir = path.resolve(
  repoRoot,
  'packages',
  'showcase',
  'app',
...
function generateTemplatesRegistrySource(manifests) {
  ...
  lines.push('// Generated by scripts/generate-showcase-registry.cjs based on web-builder manifests.');
  ...
  for (const m of manifests) {
    for (const segment of m.segments || []) {
      const themes = m.themes && m.themes[segment];
  ...
  for (const m of manifests) {
    ...
    for (const seg of segs) {
      const themes = (m.themes && m.themes[seg]) || [];
