...
function generateTemplatesRegistrySource(manifests: Manifest[]): string {
  ...
  lines.push("import type { ExtraArtifactsJSON } from '@kiskadee/web-builder/types';");
  ...
  for (const m of manifests) {
    ...
    const entry = {
      displayName,
      components: Object.fromEntries(Object.keys(m.components ?? {}).map((name) => [name, true]))
    };
...
function generateCssRegistrySource(manifests: Manifest[]): string {
  ...
  for (const m of manifests) {
    const _coreCss = `${showcaseBuildBase}/${m.key}/core.kiskadee.css`;
    const _effectsCss = `${showcaseBuildBase}/${m.key}/effects.kiskadee.css`;
